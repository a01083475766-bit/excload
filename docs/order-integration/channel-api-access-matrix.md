...
16. **실사용 전 심사**: 내 스토어 앱은 심사 없음. **통합매니저 권한** 스토어만 앱 등록 가능. 스토어당 앱 **최대 3개**
17. **API 호출 제한**: 커머스API Rate Limit
18. **엑클로드 구현 일치**: `authType: ['oauth2','ip_whitelist']`, 전자서명 방식, upstream `api.commerce.naver.com`. 공식과 일치
19. **현재 안내 문제점**: 현재 안내는 "커머스API 애플리케이션을 등록하고 IP 추가"로만 서술 → **내 스토어 앱 vs 커머스솔루션마켓** 경로 구분 없음. "URL 입력 불필요"는 사실이나 명시 필요. 스토어당 앱 3개·통합매니저 권한 조건 누락
...
- **조건**: 스토어의 **통합매니저 권한** 보유자만 등록. **스토어당 최대 3개** 앱
...
| 스토어당 애플리케이션 최대 개수 | **최대 3개** (공식 공지) | 확정 |
