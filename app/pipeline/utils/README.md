...
- **Stage2**: 입력 데이터를 `BASE_HEADERS` 기준으로 각 입력 소스별로 독립 표준화합니다. 이 단계에서는 외부 데이터(다른 파일/다른 Stage2 결과) 참조 및 병합을 수행하지 않습니다.
- **Stage3**: 주문/송장 등 복수 Stage2 결과의 모든 데이터 병합과 템플릿 매핑은 이 단계에서만 수행합니다.
