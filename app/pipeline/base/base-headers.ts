 ...
 * - UI 노출 금지
 * - 한글 컬럼 기준 구조
 * - 코어: 위 항목 + 쇼핑몰·쿠팡/사이소 확장 열(주문상태·상품주문번호·할인/포인트·배송첨부 등) → 코어 66개 + v3 3PL 10개 = 총 76개
 */
...
export const BASE_HEADERS = [
  ...
  '주문자연락처',
  '주문일시',
  /** 결제 완료 시각. 나중에결제(무통장 등)는 주문일시와 다를 수 있어 분리 */
  '결제일시',
  '결제금액',
  ...
  '택배사',
  /** 택배/등기/소포, 퀵서비스, 방문수령, 직접전달 등 배송수단 구분 (송장 필수여부·자동배송완료 여부 판단에 사용) */
  '배송방법',
  '묶음배송번호',
  '분리배송여부',
  '분리배송출고예정일',
...
export const CORE_BASE_HEADER_COUNT = 66;

/** 물류·3PL 보조 필드 (코어 66개 다음). 텍스트 변환 시 라벨이 명시될 때만 채움(프롬프트 규칙). */
