...
import {
  ...
  markDomeggookAccountTestResult,
...
export async function POST() {
  ...
  try {
    ...
    const result = await testDomeggookConnection({ credentials });
    await markDomeggookAccountTestResult({
      accountId: account.id,
      userId: auth.userId,
    ...
    return NextResponse.json({
      ...
      message:
        result.orderCount > 0
          ? `도매꾹 API 연결이 정상 확인되었습니다. (최근 1일 판매 주문 ${result.orderCount}건)`
          : '도매꾹 API 연결이 정상 확인되었습니다. (최근 1일 판매 주문 0건)',
