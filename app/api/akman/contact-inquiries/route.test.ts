...
function deleteRequest(body: unknown) {
  return new Request('http://localhost/api/akman/contact-inquiries', {
    method: 'DELETE',
    body: JSON.stringify(body),
  }) as never;
}

describe('DELETE /api/akman/contact-inquiries', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('관리자가 아니면 삭제할 수 없다', async () => {
    ...
    const res = await DELETE(deleteRequest({ ids: ['inquiry-1'] }));

    expect(res.status).toBe(403);
    expect(mocks.deleteMany).not.toHaveBeenCalled();
  });

  it('선택한 고객문의를 일괄 삭제한다', async () => {
    const res = await DELETE(deleteRequest({ ids: ['inquiry-1', 'inquiry-2', 'inquiry-1', ''] }));
    ...
    expect(mocks.deleteMany).toHaveBeenCalledWith({
      where: { id: { in: ['inquiry-1', 'inquiry-2'] } },
    });
  });
