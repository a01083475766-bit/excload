...
export default function HistoryPage() {
  ...
  const handleRedownload = (session: HistorySession, e: React.MouseEvent) => {
    ...
    try {
      ...
      const defaultFileName = `엑클로드주문정리 ${yy}년${month}월${day}일${hour}시${minute}분.xlsx`;
      const fileName = session.downloadedFileName || defaultFileName;
    ...
    <div className="pt-12 bg-zinc-50 dark:bg-black min-h-screen">
      <main className="max-w-[1200px] mx-auto px-3 sm:px-5 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          {/* 보관 정책 안내 — 변환 내역만 브라우저 보관(설정·양식·매핑은 계정 DB 예정) */}
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 leading-relaxed">
            변환 내역은 이 브라우저에 20일 동안만 저장됩니다.
            <br />
            방문 기록·쿠키·캐시·사이트 데이터를 삭제하면 변환 내역이 사라질 수 있으니,
            필요한 결과 파일은 미리 다운로드해 주세요.
          </p>
        </div>
