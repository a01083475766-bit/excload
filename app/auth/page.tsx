...
function AuthPageContent() {
  ...
  /** Google OAuth 리다이렉트 직전 중복 클릭 방지 */
  ...
  const getAuthErrorMessage = (errorCode: string | null): string => {
    switch (errorCode) {
      ...
      case 'OAuthCallback':
        return 'Google 로그인 콜백 단계에서 실패했습니다. 주소창이 www.excload.com이면 excload.com으로 접속한 뒤 다시 시도해주세요. 계속되면 Vercel의 NEXTAUTH_URL이 https://excload.com 과 일치하는지 확인해주세요.';
      case 'OAuthSignin':
  ...
  return (
    <>
    <div className="min-h-screen flex items-start justify-center bg-white px-4 pt-12 pb-10 sm:pt-20">
      <div className="w-full max-w-md">
        <div className="bg-white p-2 sm:p-6">
          <div className="mb-10 text-center">
            <Link href="/excload" className="inline-flex items-center gap-2 text-gray-700">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg border-2 border-gray-700 text-sm font-black">
                E
              </span>
            ...
            <h1 className="mt-10 text-4xl font-black tracking-tight text-gray-950 sm:text-5xl">
              {mode === 'signup' ? '이메일로 시작하기' : showEmailLoginForm ? '로그인' : '1초만에 시작하기'}
            </h1>
          </div>
          ...
          {mode === 'login' && !showEmailLoginForm && (
            <div className="space-y-3">
              <button
                ...
                className="relative flex w-full items-center justify-center gap-3 rounded-lg border border-[#FEE500] bg-[#FEE500] px-4 py-4 text-base font-bold text-[#191919] transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isGoogleRedirecting ? (
                  <>
              ...
              <button
                ...
                disabled={isLoading || isGoogleRedirecting}
                aria-busy={isGoogleRedirecting}
                className="relative flex w-full items-center justify-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-4 text-base font-bold text-gray-900 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isGoogleRedirecting ? (
                  <>
                    <Loader2 className="h-5 w-5 shrink-0 animate-spin" aria-hidden />
                  ...
                  <>
                    <span className="text-xl font-black leading-none text-[#03C75A]">N</span>
                    <span>네이버로 시작하기</span>
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-4 text-base font-bold text-gray-900 transition hover:bg-gray-50"
              >
                <Mail className="h-5 w-5" />
                <span>이메일로 회원가입</span>
              </button>

              <div className="pt-8 text-center text-base text-gray-400">
                이미 계정이 있으신가요?{' '}
                <button
                  type="button"
          ...
          {mode === 'login' && showEmailLoginForm && (
            <form onSubmit={handleLogin} className="space-y-4">
              <button
                ...
                className="mb-2 text-sm font-medium text-gray-500 hover:text-gray-900 hover:underline"
              >
                시작 화면으로 돌아가기
              </button>
              <div>
