...
import {
  ELEVEN_GUIDE_FOOTER,
  getElevenGuideSteps,
  type ElevenChecklistStep,
} from '@/app/lib/order-integration/eleven-visual-guide';
...
/** 11번가: IP 직접입력 단일 경로 → 「네, 다음」쌓기 */
export function ElevenChecklistGuide({ density = 'roomy', className = '' }: Props) {
  ...
  function renderDone(step: ElevenChecklistStep, displayNo: number, onJump: () => void) {
    return (
      <button
        type="button"
        key={`done-${step.id}`}
      ...
      <div className="space-y-2.5">
        ...
        {allDone ? (
          ...
          >
            ...
            <p className="mt-1 text-xs leading-relaxed text-emerald-800/90">{ELEVEN_GUIDE_FOOTER}</p>
