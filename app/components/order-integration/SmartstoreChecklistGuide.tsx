...
import {
  SMARTSTORE_API_CENTER_HREF,
  SMARTSTORE_GUIDE_FOOTER,
  SMARTSTORE_PATH_CHOICES,
  getSmartstorePathSteps,
  getSmartstoreStartStep,
  type ChecklistConfirmStep,
  type SmartstoreGuidePath,
} from '@/app/lib/order-integration/smartstore-visual-guide';
...
/**
 * 초보용 따라하기: 시작 → 화면 3장으로 경로 선택 → 「네, 다음」쌓기.
 * 완료 단계 클릭·「이전」으로 되돌아갈 수 있다.
 */
export function SmartstoreChecklistGuide({ density = 'roomy', className = '' }: Props) {
  const startStep = getSmartstoreStartStep();
  const [phase, setPhase] = useState<Phase>('start');
  const [path, setPath] = useState<SmartstoreGuidePath | null>(null);
  const [confirmed, setConfirmed] = useState(0);
  const [preview, setPreview] = useState<{ src: string; alt: string } | null>(null);
  const activeRef = useRef<HTMLDivElement | null>(null);

  const pathSteps = path ? getSmartstorePathSteps(path) : [];
  ...
  function choosePath(next: SmartstoreGuidePath) {
    setPath(next);
    setConfirmed(0);
    setPhase('steps');
  }
  ...
  function renderActive(
    ...
    return (
      ...
      >
        ...
        {step.id === 'start' ? (
          <a
            href={SMARTSTORE_API_CENTER_HREF}
            target="_blank"
        ...
        {phase === 'choose' ? (
          ...
          >
            ...
            <p className="mt-1 text-xs text-zinc-500">
              글보다 그림이 내 화면과 비슷한 쪽을 고르면 됩니다.
            </p>
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              {SMARTSTORE_PATH_CHOICES.map((choice) => (
                <div
                  key={choice.path}
                ...
                >
                  <button
                    type="button"
                    onClick={() =>
                      setPreview({ src: choice.imageSrc, alt: choice.imageAlt })
                    }
        ...
        {allDone ? (
          ...
          >
            <p className="text-sm font-semibold text-emerald-900">
              {path === 'blocked' ? '안내를 확인했습니다' : '설정 따라하기를 모두 마쳤습니다'}
            </p>
            <p className="mt-1 text-xs leading-relaxed text-emerald-800/90">
              {path === 'blocked'
                ? '권한·스토어 상태를 스마트스토어센터 또는 네이버 고객센터에서 해결한 뒤 다시 시작해 주세요.'
                : SMARTSTORE_GUIDE_FOOTER}
            </p>
