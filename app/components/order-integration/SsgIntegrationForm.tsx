...
{!embedded ? (
      <p className="mb-6 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
        API 인증키 저장 후 연결 테스트를 진행할 수 있습니다. 실제 주문 조회·수집(배송지시·출고대상)은 주문연동
...
{!embedded ? (
      <CollapsibleGuide title="API 발급 방법 보기 (SSG.COM)">
      ...
      <form className="mt-6 space-y-4" onSubmit={(e) => e.preventDefault()}>
        ...
        <SecretInput
          ...
          newPlaceholder="API 인증키 입력 (Authorization 헤더, 저장 후 전체 노출되지 않습니다)"
      ...
      {savedAccount ? <IntegrationConnectedNotice mallName="SSG.COM" /> : null}
    </div>
  );
}
