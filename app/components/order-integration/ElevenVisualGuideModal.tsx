...
import { ElevenChecklistGuide } from '@/app/components/order-integration/ElevenChecklistGuide';
import {
...
/** 「설정 따라하기」 — 11번가 OPEN API 그림 가이드 (IP 직접입력) */
export function ElevenVisualGuideModal({ open, onClose }: Props) {
  return (
    <WorkspaceBlockingModalOverlay
      ...
      aria-labelledby="eleven-visual-guide-title"
    >
      <div className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-zinc-200 px-5">
        <h2 id="eleven-visual-guide-title" className={EXCLOAD_MODAL_TITLE}>
          11번가 설정 따라하기
        </h2>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {open ? <ElevenChecklistGuide density="roomy" key="eleven-guide-open" /> : null}
      </div>
