...
import { SmartstoreChecklistGuide } from '@/app/components/order-integration/SmartstoreChecklistGuide';
import {
...
/** 「설정 따라하기」 — 그림 가이드를 크게 표시 */
export function SmartstoreVisualGuideModal({ open, onClose }: Props) {
  return (
    <WorkspaceBlockingModalOverlay
      ...
      aria-labelledby="smartstore-visual-guide-title"
    >
      <div className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-zinc-200 px-5">
        <h2 id="smartstore-visual-guide-title" className={EXCLOAD_MODAL_TITLE}>
          스마트스토어 설정 따라하기
        </h2>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {open ? <SmartstoreChecklistGuide density="roomy" key="guide-open" /> : null}
      </div>
