...
export function NormalizeQualityNoticeModal({
  ...
  return (
    ...
    >
      <div
        className={`${EXCLOAD_MODAL_PANEL} max-w-[480px]`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="border-b border-zinc-100 px-6 pb-4 pt-5">
          <h3 id="normalize-quality-notice-title" className={EXCLOAD_MODAL_TITLE}>
            {title}
          </h3>
        </div>
        <div className={`space-y-3 px-6 py-5 ${EXCLOAD_MODAL_BODY}`}>
          {variant === 'network' && (
            <>
              <p>
                인터넷 연결이 불안정하거나 일시적으로 서버에 닿지 못한 것 같습니다. 주문 자동
                정리는 안정적인 네트워크 연결이 필요합니다.
              </p>
              <p>
          ...
          {variant === 'timeout' && (
            <>
              ...
              <p>줄바꿈·탭으로 주문을 나눠 붙이거나, 잠시 후 다시 시도해 보세요.</p>
              <p className="text-zinc-500">{NO_CHARGE_NOTE}</p>
            </>
          )}
          {variant === 'convert_failed' && (
          ...
          {variant === 'heuristic' && (
            <>
              ...
              <p>
                ...
                이 맞는지 꼭 확인한 뒤 다운로드해 주세요.
              </p>
              <p className="text-zinc-500">
                빠진 항목이 있으면 미리보기에서 직접 수정하거나, 줄바꿈·탭으로 정리한 뒤 다시
                변환해 보세요.
              </p>
            </>
          )}
