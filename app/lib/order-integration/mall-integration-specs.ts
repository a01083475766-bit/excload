...
export const CHANNEL_INTEGRATION_SPECS: ChannelIntegrationSpec[] = [
  ...
  },
  {
    ...
    proxyDomains: [
      proxyDomain('openapi.lotteon.com', ['https'], 'planned'),
    ],
    requiredInputs: [
      { key: 'accountName', label: '계정명', required: true, storage: 'accountName' },
      ...
      { key: 'shopId', label: 'Shop ID', required: false, storage: 'sellerId' },
    ],
    memo: '종합몰 direct 후보. API Key는 Query Key 파라미터.',
  ...
  },
  {
    ...
    tokenExpirePolicy: 'partner_key + key(쇼핑몰별) — POST XML, OAuth 없음',
    rateLimitMemo: 'Token Bucket — 1초 100회 초과 시 429, ratelimit-available-level 헤더',
    proxyDomains: [
      proxyDomain('openhub.godo.co.kr', ['https'], 'planned'),
      proxyDomain('sbopenhub.godo.co.kr', ['http'], 'planned', 'sandbox'),
    ],
    requiredInputs: [
      { key: 'accountName', label: '계정명', required: true, storage: 'accountName' },
      { key: 'mallDomain', label: '쇼핑몰 도메인', required: true, storage: 'vendorId' },
      { key: 'partnerKey', label: '제휴사키(partner_key)', required: true, secret: true, storage: 'accessKey' },
      { key: 'userKey', label: '사용자키(key)', required: true, secret: true, storage: 'apiKey' },
    ],
    memo: 'Order_Search.php 등 POST XML. 호스팅몰은 NHN openhub IP 방화벽 별도.',
  ...
  },
  {
    ...
    phase: 'planned',
    requiredSellerAction:
      '샵바이 서비스어드민 → 개발연동정보 → clientId·외부연동키(mallKey). 워크스페이스 앱 systemKey 발급',
    tokenExpirePolicy: 'systemKey + mallKey 또는 OAuth — Server API 인증',
    rateLimitMemo: 'Server API — 공식 문서 Rate Limit',
    proxyDomains: [
      proxyDomain('server-api.e-ncp.com', ['https'], 'planned', '주문·관리 Server API'),
      proxyDomain('shop-api.e-ncp.com', ['https'], 'planned', 'Shop API — 필요 시'),
    ],
    requiredInputs: [
      { key: 'accountName', label: '계정명', required: true, storage: 'accountName' },
      { key: 'clientId', label: 'clientId', required: true, storage: 'accessKey' },
    ...
    memo: '자사몰·독립몰. Server API 우선. API outbound IP whitelist 없음.',
